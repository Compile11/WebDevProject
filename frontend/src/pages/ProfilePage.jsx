...
export default function ProfilePage() {
  ...
  const hasChanges =
    usernameField !== currentUser.username ||
    bioField !== currentUser.bio ||
    profilePicFile !== null;
            ...
            {isEditing ? (
              <label className="group relative w-full h-full cursor-pointer overflow-hidden rounded-full flex items-center justify-center text-5xl font-bold text-white shadow-inner block">
                {profilePicFile ? (
                  <img
                    ...
                    alt="Preview"
                    className="w-full h-full object-cover border-4 border-blue-500 rounded-full"
                  />
                ) : currentUser?.profilePic ? (
                  <img
                    src={currentUser.profilePic}
                    alt="Profile"
                    className="w-full h-full object-cover border-4 border-transparent group-hover:border-blue-500 transition-all rounded-full"
                  />
                ) : (
              ...
              >
                {currentUser?.profilePic ? (
                  ...
                  <div className="flex items-center absolute inset-0 bg-blue-600 flex items-center justify-center">
            ...
            {isEditing ? (
              <div className="flex flex-col gap-2 max-w-md">
                ...
                <div className="flex gap-2 mt-2">
                  <button
                    onClick={handleSave}
                    className="flex-1 bg-green-600 hover:bg-green-700 text-white font-bold py-1.5 rounded flex items-center justify-center gap-1 text-sm"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 bg-gray-600 text-white py-1.5 rounded text-sm"
                  >
                    Cancel
                  </button>
                </div>
